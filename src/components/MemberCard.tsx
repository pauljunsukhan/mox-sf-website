...
export function MemberCard({ member }: { member: Member }) {
  ...
  const isStaff = member.category === 'staff'

  // One spec row per category for non-staff cards — keeps them uncrowded.
  const details = (
  ...
  return (
    <article className={`card card--${member.category}`}>
        ...
        <div className="card-face card-front">
          ...
          {isStaff ? (
            <div className="card-staff">
              {member.role && <div className="card-role">{member.role}</div>}
              {member.interestText && (
              ...
              {member.contactFor.length > 0 && (
                <div className="card-contact">
                  <span className="card-contact-label">Contact for</span>
                  <ul className="card-contact-list">
                    {member.contactFor.map((item) => (
                      <li key={item}>{item}</li>
                    ))}
                  </ul>
                </div>
